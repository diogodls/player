...
const IndividualAnalysis = () => {
  ...
  } = useApi<AnalysisPlayerListResponse>(
    sessionId ? "/players?limit=100" : null,
  );
  const {
    data: catalog,
  ...
  if (isSessionLoading || arePlayersLoading || isCatalogLoading) {
    return (
      <div className={styles.feedback}>Carregando análise individual...</div>
    );
  }
    ...
    <div className={styles.container}>
      ...
      {exitGuard.isExitModalOpen && (
        <AnalysisExitModal
          ...
          isProcessing={exitGuard.isProcessingExit}
        />
      )}
    </div>
  );
